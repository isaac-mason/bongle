// performance: terrain.
//
// A 512 by 64 by 512 world with rolling hills, lakes, forests and scattered
// buildings, giving high geometric variance across every voxel render path:
//   opaque      stone, dirt, grass, gravel, snow, cobblestone, planks, logs
//               and wool: a varied palette with lots of material changes.
//   transparent glass windows and leaf canopies (alpha-cutout).
//   translucent lake water (the back-to-front sorted pass).
// So it exercises streaming, culling, meshing and all three draw passes at map
// scale. Generation is deterministic (hash noise, no RNG) so runs are
// reproducible for A/B perf comparisons. `size` and the other controls apply
// on room restart.
...
// Deterministic 2D hash in [0, 1). This is the RNG-free noise source.
...
function placeTree(voxels: Voxels, x: number, z: number, groundY: number, lo: number, hi: number): void {
    const trunk = 4 + Math.floor(hash2(x * 7, z * 13) * 3); // 4 to 6
    const topY = groundY + trunk;
    for (let y = groundY + 1; y <= topY; y++) setBlock(voxels, x, y, z, log, BULK);
    // canopy: 5 by 5 lower rings tapering to a 3 by 3 cap.
...
function placeBuilding(voxels: Voxels, cx: number, cz: number, floorY: number, seed: number): void {
    const half = 3 + (seed % 2); // 3 or 4, so 7 by 7 or 9 by 9
    const floors = 1 + (seed % 3); // 1 to 3
...
const TerrainTrait = trait('terrain', {
    /** map footprint on X/Z, in blocks. 512 is 32 by 32 chunks. */
    size: 512,
...
control(TerrainTrait, 'treePerMille', {
    label: 'Tree density (per mille, restart)',
    schema: prop.number(),
...
script(TerrainTrait, 'generate', (ctx) => {
    ...
    onInit(ctx, () => {
        ...
        // Terrain and water: per-chunk tier-1 fill. Resolve each chunk once,
        // grab its palette slots once, and write voxel data straight into the
        // chunk's typed array. invalidateChunk then reconciles counts and
        // schedules the scoped relight. This skips the per-block chunk-key
        // lookup, op, and light-queue work entirely, which is the bulk of the
        // speedup.
        const ccLo = lo >> 4;
        const ccHi = hi >> 4;
        for (let cx = ccLo; cx <= ccHi; cx++) {
            for (let cz = ccLo; cz <= ccHi; cz++) {
                // precompute this chunk column's 16 by 16 heights and surface once.
                ...
                for (let lz = 0; lz < CHUNK_SIZE; lz++) {
                    for (let lx = 0; lx < CHUNK_SIZE; lx++) {
                        ...
                        if (wx < lo || wx > hi || wz < lo || wz > hi) {
                            heights[lz * CHUNK_SIZE + lx] = -1; // outside the map, so an air column
                            continue;
                        }
