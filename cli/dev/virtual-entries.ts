// cli/dev/virtual-entries.ts — serves the node-env boot modules as Vite
// virtuals. start.ts imports these through each env's runner and calls boot(ctx):
//   virtual:bongle/edit-server → realms/server/edit-server (EngineServer + /game WS)
//   virtual:bongle/pipeline    → realms/pipeline/pipeline (data bake on flush)
...
const EDIT_SERVER = fileURLToPath(new URL('../realms/server/edit-server.ts', import.meta.url));
const PIPELINE = fileURLToPath(new URL('../realms/pipeline/pipeline.ts', import.meta.url));
