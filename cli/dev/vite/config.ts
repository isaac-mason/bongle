// cli/dev/vite/config.ts — Vite config for `bongle dev` (client + server envs).
//
...
import { bongle, type EngineRebootRef } from './bongle-plugin';
...
/** bongle's package root (this file is <root>/cli/dev/vite/config.ts). */
...
export function defineBongleDevConfig(opts: BongleDevConfigOptions): UserConfig {
    ...
    return defineConfig({
        ...
        environments: {
            ...
            // flush (see runtime/pipeline.ts); no icons (no Dawn in this process).
            pipeline: {
