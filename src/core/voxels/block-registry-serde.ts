...
export function serializeBlockRegistryForWorker(reg: BlockRegistry, version: number): ArrayBuffer {
    ...
    c = align8(c); const oMeshQuadCount = c; c += (meshCount + 1) * 2;

    c = align8(c); const oCull            = c; c += totalStates * 1;
    c = align8(c); const oBlockTypeId     = c; c += totalStates * 2;
    c = align8(c); const oMaterial        = c; c += totalStates * 1;
    c = align8(c); const oModelType       = c; c += totalStates * 1;
    c = align8(c); const oCubeTexIndices  = c; c += totalStates * 6 * 2;
    c = align8(c); const oCubeFaceUVs     = c; c += totalStates * 48 * 1;
    c = align8(c); const oMeshId          = c; c += totalStates * 2;
    c = align8(c); const oVertexAnimation = c; c += totalStates * 1;
    c = align8(c); const oSurfaceHeight   = c; c += totalStates * 4;
    c = align8(c); const oFluidGroup      = c; c += totalStates * 2;
    c = align8(c); const oEmissive        = c; c += totalStates * 1;

    c = align8(c); const oMeshTexIndices       = c; c += totalQuads * STRIDE_MESH_TEX_INDICES * 2;
    c = align8(c); const oMeshQuadMaterials    = c; c += totalQuads * STRIDE_MESH_QUAD_MATERIALS * 1;
    c = align8(c); const oMeshQuadShape        = c; c += totalQuads * STRIDE_MESH_QUAD_SHAPE * 1;
    c = align8(c); const oMeshQuadFaceDir      = c; c += totalQuads * STRIDE_MESH_QUAD_FACE_DIR * 1;
    c = align8(c); const oMeshQuadCullFaceDir  = c; c += totalQuads * STRIDE_MESH_QUAD_CULL_FACE_DIR * 1;
    c = align8(c); const oMeshQuadDepth        = c; c += totalQuads * STRIDE_MESH_QUAD_DEPTH * 4;
    c = align8(c); const oMeshQuadVertDepth    = c; c += totalQuads * STRIDE_MESH_QUAD_VERT_DEPTH * 4;
    c = align8(c); const oMeshQuadVertNormal   = c; c += totalQuads * STRIDE_MESH_QUAD_VERT_NORMAL * 4;
    c = align8(c); const oMeshQuadCornerUV     = c; c += totalQuads * STRIDE_MESH_QUAD_CORNER_UV * 4;
    c = align8(c); const oMeshQuadCornerPos    = c; c += totalQuads * STRIDE_MESH_QUAD_CORNER_POS * 4;
    c = align8(c); const oMeshQuadCornerNormSq = c; c += totalQuads * STRIDE_MESH_QUAD_CORNER_NORM_SQ * 4;
    c = align8(c); const oMeshQuadNormal       = c; c += totalQuads * STRIDE_MESH_QUAD_NORMAL * 4;
    c = align8(c); const oMeshQuadUVs          = c; c += totalQuads * STRIDE_MESH_QUAD_UVS * 4;
    c = align8(c); const oMeshQuadVerts        = c; c += totalQuads * STRIDE_MESH_QUAD_VERTS * 4;
    ...
    for (let m = 1; m <= meshCount; m++) {
        const t = reg.meshTexIndices[m]!;        copyBytes(u8, t, qti); qti += t.byteLength;
        const mm = reg.meshQuadMaterials[m]!;    copyBytes(u8, mm, qmm); qmm += mm.byteLength;
        const ms = reg.meshQuadShape[m]!;        copyBytes(u8, ms, qms); qms += ms.byteLength;
        const mf = reg.meshQuadFaceDir[m]!;      copyBytes(u8, mf, qmf); qmf += mf.byteLength;
        const mc = reg.meshQuadCullFaceDir[m]!;  copyBytes(u8, mc, qmc); qmc += mc.byteLength;
        const md = reg.meshQuadDepth[m]!;        copyBytes(u8, md, qmd); qmd += md.byteLength;
        const mvd = reg.meshQuadVertDepth[m]!;   copyBytes(u8, mvd, qmvd); qmvd += mvd.byteLength;
        const mvn = reg.meshQuadVertNormal[m]!;  copyBytes(u8, mvn, qmvn); qmvn += mvn.byteLength;
        const mcu = reg.meshQuadCornerUV[m]!;    copyBytes(u8, mcu, qmcu); qmcu += mcu.byteLength;
        const mcp = reg.meshQuadCornerPos[m]!;   copyBytes(u8, mcp, qmcp); qmcp += mcp.byteLength;
        const mcn = reg.meshQuadCornerNormSq[m]!; copyBytes(u8, mcn, qmcn); qmcn += mcn.byteLength;
        const mn = reg.meshQuadNormal[m]!;       copyBytes(u8, mn, qmn); qmn += mn.byteLength;
        const mu = reg.meshQuadUVs[m]!;          copyBytes(u8, mu, qmu); qmu += mu.byteLength;
        const mv = reg.meshQuadVerts[m]!;        copyBytes(u8, mv, qmv); qmv += mv.byteLength;
    }
...
export function deserializeBlockRegistryForWorker(buf: ArrayBuffer): DeserializedBlockRegistry {
    ...
    for (let m = 1; m <= meshCount; m++) {
        ...
        meshTexIndices[m]       = new Uint16Array (buf, qti,  n * STRIDE_MESH_TEX_INDICES);       qti  += n * STRIDE_MESH_TEX_INDICES * 2;
        meshQuadMaterials[m]    = new Uint8Array  (buf, qmm,  n * STRIDE_MESH_QUAD_MATERIALS);    qmm  += n * STRIDE_MESH_QUAD_MATERIALS;
        meshQuadShape[m]        = new Uint8Array  (buf, qms,  n * STRIDE_MESH_QUAD_SHAPE);        qms  += n * STRIDE_MESH_QUAD_SHAPE;
        meshQuadFaceDir[m]      = new Uint8Array  (buf, qmf,  n * STRIDE_MESH_QUAD_FACE_DIR);     qmf  += n * STRIDE_MESH_QUAD_FACE_DIR;
        meshQuadCullFaceDir[m]  = new Uint8Array  (buf, qmc,  n * STRIDE_MESH_QUAD_CULL_FACE_DIR);qmc  += n * STRIDE_MESH_QUAD_CULL_FACE_DIR;
        meshQuadDepth[m]        = new Float32Array(buf, qmd,  n * STRIDE_MESH_QUAD_DEPTH);        qmd  += n * STRIDE_MESH_QUAD_DEPTH * 4;
        meshQuadVertDepth[m]    = new Float32Array(buf, qmvd, n * STRIDE_MESH_QUAD_VERT_DEPTH);   qmvd += n * STRIDE_MESH_QUAD_VERT_DEPTH * 4;
        meshQuadVertNormal[m]   = new Float32Array(buf, qmvn, n * STRIDE_MESH_QUAD_VERT_NORMAL);  qmvn += n * STRIDE_MESH_QUAD_VERT_NORMAL * 4;
        meshQuadCornerUV[m]     = new Float32Array(buf, qmcu, n * STRIDE_MESH_QUAD_CORNER_UV);    qmcu += n * STRIDE_MESH_QUAD_CORNER_UV * 4;
        meshQuadCornerPos[m]    = new Float32Array(buf, qmcp, n * STRIDE_MESH_QUAD_CORNER_POS);   qmcp += n * STRIDE_MESH_QUAD_CORNER_POS * 4;
        meshQuadCornerNormSq[m] = new Float32Array(buf, qmcn, n * STRIDE_MESH_QUAD_CORNER_NORM_SQ); qmcn += n * STRIDE_MESH_QUAD_CORNER_NORM_SQ * 4;
        meshQuadNormal[m]       = new Float32Array(buf, qmn,  n * STRIDE_MESH_QUAD_NORMAL);       qmn  += n * STRIDE_MESH_QUAD_NORMAL * 4;
        meshQuadUVs[m]          = new Float32Array(buf, qmu,  n * STRIDE_MESH_QUAD_UVS);          qmu  += n * STRIDE_MESH_QUAD_UVS * 4;
        meshQuadVerts[m]        = new Float32Array(buf, qmv,  n * STRIDE_MESH_QUAD_VERTS);        qmv  += n * STRIDE_MESH_QUAD_VERTS * 4;
    }
