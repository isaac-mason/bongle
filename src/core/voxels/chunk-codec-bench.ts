...
const packLight = (sky: number, r: number, g: number, b: number) =>
    (sky << 12) | (r << 8) | (g << 4) | b;
...
function rleEncode(input: Uint16Array): Uint16Array {
    ...
    let n = 0, rv = input[0]!, rl = 1;
    for (let i = 1; i < input.length; i++) {
        const v = input[i]!;
        if (v === rv && rl < 65535) rl++;
        else { pairs[n++] = rv; pairs[n++] = rl; rv = v; rl = 1; }
    }
    pairs[n++] = rv; pairs[n++] = rl;
...
function splitLight(light: Uint16Array): { sky: Uint16Array; rgb: Uint16Array } {
    const sky = new Uint16Array(light.length), rgb = new Uint16Array(light.length);
...
type Codec = {
    name: string;
    run: (data: Uint16Array, light: Uint16Array) => {
        bytes: number;
...
const A_baseline: Codec = {
    ...
    run(data, light) {
        ...
        for (let i = 0; i < CHUNK_VOLUME; i++) { inter[i * 2] = data[i]!; inter[i * 2 + 1] = light[i]!; }
        ...
        return {
            ...
            decode() {
                ...
                const d = new Uint16Array(CHUNK_VOLUME), l = new Uint16Array(CHUNK_VOLUME);
                for (let i = 0; i < CHUNK_VOLUME; i++) { d[i] = flat[i * 2]!; l[i] = flat[i * 2 + 1]!; }
...
const D_splitDeflateOnce: Codec = {
    ...
    run(data, light) {
        const { sky, rgb } = splitLight(light);
        const dR = u16Bytes(rleEncode(data)), sR = u16Bytes(rleEncode(sky)), rR = u16Bytes(rleEncode(rgb));
        const concat = new Uint8Array(12 + dR.length + sR.length + rR.length);
        const dv = new DataView(concat.buffer);
        dv.setUint32(0, dR.length); dv.setUint32(4, sR.length); dv.setUint32(8, rR.length);
        concat.set(dR, 12); concat.set(sR, 12 + dR.length); concat.set(rR, 12 + dR.length + sR.length);
        ...
        return {
            ...
            decode() {
                ...
                const dl = dv2.getUint32(0), sl = dv2.getUint32(4), rl = dv2.getUint32(8);
                let off = 12;
                const dR2 = raw.slice(off, off + dl); off += dl;
                const sR2 = raw.slice(off, off + sl); off += sl;
...
const E_bitpackIdx: Codec = {
    ...
    run(data, light) {
        ...
        const { sky, rgb } = splitLight(light);
        const sR = u16Bytes(rleEncode(sky)).slice(), rR = u16Bytes(rleEncode(rgb)).slice();
        return {
            ...
            decode() {
                ...
                for (let i = 0; i < CHUNK_VOLUME; i++) {
                    let v = 0;
                    for (let b = 0; b < bits; b++) { v |= ((packed[bp >> 3]! >> (bp & 7)) & 1) << b; bp++; }
                    dd[i] = v;
                }
                const ss = rleDecode(new Uint16Array(sR.buffer), CHUNK_VOLUME);
                const rr = rleDecode(new Uint16Array(rR.buffer), CHUNK_VOLUME);
...
const D2_combinedLightDeflate: Codec = {
    ...
    run(data, light) {
        const dR = u16Bytes(rleEncode(data)), lR = u16Bytes(rleEncode(light));
        const concat = new Uint8Array(8 + dR.length + lR.length);
        const dv = new DataView(concat.buffer);
        dv.setUint32(0, dR.length); dv.setUint32(4, lR.length);
        concat.set(dR, 8); concat.set(lR, 8 + dR.length);
        const compressed = deflateSync(concat);
        return {
            bytes: compressed.length,
            decode() {
                const raw = inflateSync(compressed);
                const dv2 = new DataView(raw.buffer, raw.byteOffset);
                const dl = dv2.getUint32(0), ll = dv2.getUint32(4);
...
const CODECS = [A_baseline, B_splitNoDeflate, D_splitDeflateOnce, D2_combinedLightDeflate, D3_splitNoRleDeflate, D4_rawDeflate, E_bitpackIdx];
...
const SCENES = [
    sceneUniformAir(), sceneSolidStone(), sceneSurface(), sceneLayered(),
    sceneCaveEmitter(), sceneMixedBuild(), sceneNoisy(),
];
...
// ── run ─────────────────────────────────────────────────────────────
const ENC_ITERS = 2000, DEC_ITERS = 5000;
...
for (const scene of SCENES) {
    ...
    for (const codec of CODECS) {
        ...
        const encUs = timeUs(() => { codec.run(scene.data, scene.light); }, ENC_ITERS);
        const decUs = timeUs(() => { built.decode(); }, DEC_ITERS);
