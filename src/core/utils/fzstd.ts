...
const slc = (v: Uint8Array, s: number, e?: number): Uint8Array => v.slice(s, e);

const fill = (v: Uint8Array, n: number, s?: number, e?: number): Uint8Array => v.fill(n, s, e);

const cpw = (v: Uint8Array, t: number, s: number, e?: number): void => {
    v.copyWithin(t, s, e);
};
...
const err = (ind: ZEC, msg?: string): never => {
...
const rzfh = (dat: Uint8Array, w?: Uint8Array | 1): number | DZstdState => {
    ...
    if (n3 === 0x2fb528 && dat[3] === 253) {
        ...
        // frame source size
        const fss = rb(dat, bt, fsb) + (fcf === 1 ? 256 : 0);
        // window size
        let ws = fss;
        if (!ss) {
            // window descriptor
...
const rhu = (dat: Uint8Array, bt: number): [number, HDT] => {
    ...
    for (i = 0; i < wc; ++i) {
        ...
        if (bits) {
            const code = ri[bits];
            const next = code + (1 << (mb - bits));
            ri[bits] = next;
            fill(syms, i, code, next);
        }
    }
    return [
        bt,
...
const dhu4 = (dat: Uint8Array, out: Uint8Array, hu: HDT) => {
    ...
    let s0 = bt;
    bt += dat[0] | (dat[1] << 8);
    dhu(dat.subarray(s0, bt), out.subarray(0, sz1), hu);
    s0 = bt;
    bt += dat[2] | (dat[3] << 8);
    dhu(dat.subarray(s0, bt), out.subarray(sz1, sz2), hu);
    s0 = bt;
    bt += dat[4] | (dat[5] << 8);
    dhu(dat.subarray(s0, bt), out.subarray(sz2, sz3), hu);
    dhu(dat.subarray(bt), out.subarray(sz3), hu);
};

// read Zstandard block. returns undefined when the input is short a full block
// (caller treats that as "need more data" / unexpected EOF).
const rzb = (dat: Uint8Array, st: DZstdState, out?: Uint8Array): Uint8Array | undefined => {
    let bt = st.b;
    //    byte 0        block type
    const b0 = dat[bt],
        btype = (b0 >> 1) & 3;
    ...
    if (btype === 2) {
        ...
        if (lbt === 0) {
            const litStart = bt;
            bt += lss;
            buf.set(dat.subarray(litStart, bt), spl);
            ...
            if (lbt === 2) {
                ...
                hu = hud[1];
                st.h = hu;
            } else if (!hu) return err(0);
            const litStart = bt;
            bt += lcs;
            (s4 ? dhu4 : dhu)(dat.subarray(litStart, bt), buf.subarray(spl), hu);
        }
        // num sequences
        let ns = dat[bt++];
        if (ns) {
            ...
            for (++ns; --ns; ) {
                ...
                let off =
                    ofp +
                    (((dat[cbt] | (dat[cbt + 1] << 8) | (dat[cbt + 2] << 16) | (dat[cbt + 3] << 24)) >>> (spos & 7)) & (ofp - 1));
                spos -= mlb[mlc];
                cbt = spos >> 3;
                let ml =
                    mlbl[mlc] + (((dat[cbt] | (dat[cbt + 1] << 8) | (dat[cbt + 2] << 16)) >> (spos & 7)) & ((1 << mlb[mlc]) - 1));
                spos -= llb[llc];
                cbt = spos >> 3;
                const ll =
                    llbl[llc] + (((dat[cbt] | (dat[cbt + 1] << 8) | (dat[cbt + 2] << 16)) >> (spos & 7)) & ((1 << llb[llc]) - 1));
                ...
                // literal run: source (spl, near buf end) never overlaps the write
                // cursor, so a bulk copy is safe. copyWithin only pays past ~CWMIN bytes;
                // shorter runs stay scalar to dodge its fixed call overhead (bongle patch)
                if (ll >= CWMIN) {
                ...
                // match copy: bulk only when non-overlapping (off >= ml) and long enough.
                // overlapping matches are byte-wise RLE-style replication and MUST stay
                // scalar for correctness (bongle patch)
                if (off >= ml && ml >= CWMIN) {
            ...
            if (oubt !== spl) {
                buf.copyWithin(oubt, spl, buf.length);
                oubt += buf.length - spl;
    ...
    for (; dat.length; ) {
        ...
        if (typeof st === 'object') {
            if (nb) {
                ...
                // nb is 0 only when a buf was provided, so buf is defined here
...
/** Decode a complete Zstandard frame in one shot (decode-only). */
export function zstdDecompress(compressed: Uint8Array): Uint8Array {
