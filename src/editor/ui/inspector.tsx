...
// a pose hands translate and rotate to the gizmo.
function FrameToggle({ schema, path }: { schema: ObjectSchema; path: PropPath }) {
    ...
    if (!control || schema.kind !== 'pose') return null;
    const armed =
        activeFrame !== null &&
    ...
    return (
        <button
            type="button"
            title={armed ? 'gizmo follows the node' : 'gizmo follows this pose'}
            onClick={() => {
                if (armed) {
                    setActiveFrame(null);
                    return;
