...
export function HierarchyPanel() {
    ...
    const contextNode = contextNodeId !== null && sceneTree ? sceneTree.ids.toNode.get(contextNodeId) : null;
    ...
    useEffect(() => {
        ...
        for (const node of sceneTree.ids.toNode.values()) {
    ...
    const handleRemove = useCallback(
        (nodeId: number) => {
            if (!sceneTree) return;
            const node = sceneTree.ids.toNode.get(nodeId);
    ...
    const handleMenuCreateChild = useCallback(() => {
        ...
        const node = sceneTree.ids.toNode.get(contextNodeId);
        if (!node) return;
            ...
            <div className="p-1 border-t border-border">
                <Button
                    onClick={() => {
                        ...
                        const selectedNode = first ? sceneTree.ids.toNode.get(first) : null;
