...
// `SoundHandleMap` for typed lookup and calls `__bongle.registerSound(id, handle)`
...
function renderBarrel(entries: CodegenEntry[]): string {
    ...
    // `__bongle` is provided in module scope by the dev transform's
    // prelude (see build/dev/transform.ts), re-importing it here would
    // collide with the prelude's top-level `import { __bongle }` and parse
    // as "Identifier '__bongle' has already been declared".
    lines.push(``);
    ...
    for (const e of entries) {
        lines.push(`__bongle.registerSound(${JSON.stringify(e.id)}, ${sanitizeIdent(e.id)});`);
    }
    lines.push(``);
    lines.push(`export {};`);
    lines.push(``);
