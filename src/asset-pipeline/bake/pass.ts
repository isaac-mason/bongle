 ...
 * declaration.
 */
export function collectAssetSources(internal: PipelineInternal, projectDir: string): Set<string> {
    const out = new Set<string>();
    const add = (src: string) => out.add(resolveSrcToAbsPath(src, projectDir));
    const { registry } = internal;

    for (const h of registry.models.byId.values()) {
        if (typeof h.payload.src === 'string') add(h.payload.src);
    }
    for (const h of registry.blockTextures.byId.values()) {
        for (const frame of h.payload.frames) {
            if (typeof frame === 'string') add(frame);
        }
    }
    for (const h of registry.sprites.byId.values()) {
        const srcs = Array.isArray(h.payload.src) ? h.payload.src : [h.payload.src];
        for (const s of srcs) if (typeof s === 'string') add(s);
    }
    for (const h of registry.sounds.byId.values()) {
        if (typeof h.payload.src === 'string') add(h.payload.src);
    }

    const projectPrefix = projectDir + path.sep;
    const filtered = new Set<string>();
    for (const p of out) {
        if (p.startsWith(projectPrefix)) filtered.add(p);
    }
    return filtered;
}
