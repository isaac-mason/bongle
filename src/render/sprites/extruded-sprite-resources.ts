...
// Mirrors `ModelBatch` (see model-resources.ts): the per-slot instance buffer +
 ...
 *  the engine-global material. Not added to any scene until a room `init`s. */
function createExtrudedSpriteBatch(pool: GeometryPool, material: Material): ExtrudedSpriteBatch {
...
export function growExtrudedSpriteBatch(batch: ExtrudedSpriteBatch, newCapacity: number): void {
...
function disposeExtrudedSpriteBatch(batch: ExtrudedSpriteBatch): void {
    // pool vertex/index buffers are MANUAL (owned by the pool); the instance +
...
export function init(spriteResources: SpriteResources, env: EnvironmentResources): ExtrudedSpriteResources {
    ...
    const batch = createExtrudedSpriteBatch(geometryPool, material);
    return { material, atlasTexNode, geometryPool, spriteResources, batch };
}
...
export function dispose(res: ExtrudedSpriteResources): void {
    disposeExtrudedSpriteBatch(res.batch);
...
function createExtrudedSpriteMaterial(
    ...
    const sunAngle = mul(sub(env.timeNode.time, f32(0.25)), TAU).toVar('esSunAngle');
