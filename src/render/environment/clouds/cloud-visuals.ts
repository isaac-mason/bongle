...
export type CloudVisuals = {
    mesh: Mesh;
    scene: Scene;
    /** this room's single non-indexed instanced draw, shared by identity with
     *  `mesh.draws[0]`. `instanceCount` is overwritten each frame by the active
     *  room's cull (see `update`); vertexCount is fixed to `maxIndexCount`. */
