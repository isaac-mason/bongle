...
import type { IndexedMeshDraw, Scene } from 'gpucat';
import { d, Geometry, GpuBuffer, Mesh } from 'gpucat';
...
// ModelInstance = mat4x4f (64B) + InstanceParams (80B w/ uvOffset+uvScale) = 144B per slot.
const MODEL_INSTANCE_STRIDE_F32 = MODEL_INSTANCE_STRIDE / 4; // 36
...
export type ModelVisuals = {
     ...
     *  by its own trait version. Bound by name into the material; on WebGL the
     *  read-only storage read auto-lowers to an rgba32uint buffer-texture fetch
     *  (gpucat), so this stays a plain per-room storage buffer on both backends. */
    ...
    /** per-frame batched draw list, one entry per visible unique mesh, shared
     *  by identity with `mesh.draws`. Rebuilt in place each frame; the renderer
     *  loops it (one instanced draw per entry, each with its own firstInstance
     *  base — `instanceIndex` is base-inclusive on both backends). Replaces the
     *  WebGPU-only indirect-draw buffer (WebGL2 has no indirect draw). */
 ...
 * the scene; N instanced draws per frame (one per visible unique mesh) via
 * `mesh.draws`.
 */
export function init(scene: Scene, sceneTree: SceneTree, modelResources: ModelResources): ModelVisuals {
    ...
    // route per-room storage into the engine-global material by name. On WebGL
    // the read-only reads auto-lower to buffer-texture fetches (gpucat); env is
    // the shared uniform captured by the material (no per-geometry binding).
    ...
    // draw per visible unique mesh). Replaces the WebGPU-only indirect buffer.
...
export function dispose(visuals: ModelVisuals, visibility: Visibility.Visibility): void {
    ...
    // per-room buffers we own. `mesh.draws` is a plain array, nothing to free.
