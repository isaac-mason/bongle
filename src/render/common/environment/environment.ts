...
/**
 * Per-room environment RENDER state — owned by the renderer (built in its
 * per-room `createRoomVisuals`, disposed in `disposeRoomVisuals`). The
 * engine-global `EnvironmentResources` + `CloudResources` it draws with are NOT
 * stored here; the renderer threads them into the flush (it owns them).
 */
