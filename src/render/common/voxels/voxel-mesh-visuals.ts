...
import type { NonIndexedMeshDraw, Scene } from 'gpucat';
import { BufferLifecycle, d, Geometry, GpuBuffer, Mesh, packTo } from 'gpucat';
...
import { arenaAlloc, arenaDispose, arenaFree, arenaWrite, createSegmentArena, type SegmentArena } from './voxel-arena';
import {
    CHUNK_INFO_STRIDE,
    ChunkInfo,
    InstanceParams,
...
export type VoxelMeshVisuals = {
    ...
    /** per-frame batched draw list (non-indexed: vertex-pulled from meshQuads),
     *  one entry per (model × source-chunk) bucket, shared by identity with
     *  `mesh.draws`. Rebuilt in place each frame; the renderer loops it (one
     *  instanced draw per entry). Replaces the WebGPU-only indirect buffer. */
...
export function init(scene: Scene, sceneTree: SceneTree, voxelMeshResources: VoxelMeshResources): VoxelMeshVisuals {
    ...
    // per-frame batched draw list, rebuilt in place each frame. Replaces the
    // WebGPU-only indirect buffer (WebGL2 has no indirect draw).
...
export function update(visuals: VoxelMeshVisuals, voxels: Voxels, visibility: Visibility.Visibility): void {
    ...
    if (bucketId > 0) visuals.slotMapBuf.needsUpdate = true;
