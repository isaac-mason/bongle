// gpu-frame.ts — WebGPU-only voxel frame producer (compute cull/emit + translucent sort).
//
// Split out of the former voxel-resources.ts: this half owns the GPU compute
// chain (cull → finalize → emit → translucent radix sort), the per-pass render
// scratch (visibleQuads + indirect) + geometries, and the `VoxelResources`
// handle + init/load/dispose + the per-frame graph (updateCull / cullDispatches).
// The shared arena/packer/SectionTable/structs live in ./voxel-arena.
//
...
import type { EnvironmentResources } from '../../common/environment/environment';
import type { TimeResources } from '../../common/time';
import { createMeshDispatcher, disposeMeshDispatcher, setMeshRegistry } from '../../common/voxels/mesh-dispatcher';
import {
    arenaDispose,
    BUCKET_COUNT,
    ChunkCullRecord,
...
} from '../../common/voxels/voxel-arena';
import type { VoxelCore } from '../../common/voxels/voxel-core';
import { createQuadMaterial, decodeOct16, decodeQuadCentroid, type VoxelPass } from '../../common/voxels/voxel-material';
import {
...
} from '../../common/voxels/voxel-texture-array';
...
export function init(registry: Blocks, env: EnvironmentResources, budget: VoxelArenaBudget, time: TimeResources): VoxelResources {
    ...
    const quadMaterials: Record<VoxelPass, Material> = {
        opaque: createQuadMaterial({ atlas, texAnimBuffer, pass: 'opaque', elapsedTime, env }),
        transparent: createQuadMaterial({ atlas, texAnimBuffer, pass: 'transparent', elapsedTime, env }),
        translucent: createQuadMaterial({ atlas, texAnimBuffer, pass: 'translucent', elapsedTime, env }),
    };
...
export async function load(
    ...
    if (workerCount > 0 && typeof Worker !== 'undefined') {
        ...
        const { spawnMeshWorker } = await import('../../common/voxels/mesh-worker-spawn');
