...
export function packerClearAll(packer: ArenaPacker): void {
    ...
    // hard reset: nothing resident, so no live sort or pending self-heal.
...
export type VoxelResources = {
     ...
     *  mutation). `valid` false forces the first run (e.g. after a room swap
     *  cleared the arena). */
    ...
    /** chunk keys whose in-flight worker jobs were lost to a worker crash. Drained
     *  at the top of `voxel-visuals.update()`; each is put back on
     *  `voxels.dirty.blocks` so it gets re-dispatched next frame. */
...
/** Clear the arena: free every resident chunk + reset the sort gate, so the next
 *  world remeshes from scratch and re-sorts. Voxel DATA is untouched
 *  (`voxels.chunks`), so re-mounting simply remeshes it. Call on a room swap
 *  (the arena holds one world at a time) or teardown. */
export function clearArena(voxelResources: VoxelResources): void {
    packerClearAll(voxelResources.arenas.packer);
    // the shared translucent buffer's permutation is now stale; force a re-sort.
    voxelResources.tsortGate.valid = false;
}
