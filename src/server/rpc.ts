...
/** Sends queue onto `discovery` and drain via `Discovery.flushCommands` after the
 *  per-tick scene distribution, so a command can't beat this tick's scene state
 *  (join_room / scene_sync) onto a client's ordered socket. */
