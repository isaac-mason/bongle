// bongle/os/remote-fs — a guest's read/write Filesystem over a wire, and the host
// responder that serves it from a real Filesystem. Guest coordination the ENGINE
// owns (not the editor shell): a joining participant works THROUGH to the host's
// disk on demand, so the OS defines how that read-through is framed.
//
// A guest tab has no shared OPFS; each fs op is ONE packcat-framed binary message
// over the fsrpc channel, the host applies it to its authoritative Filesystem and
// replies. Writes are allowed: multiplayer editing is last-writer-wins (the host
// Filesystem is the single source of truth; a guest write triggers the host's
// watch → HMR + fanout to every participant).
//
...
// the same codec the engine net layer uses (packcat). It rides both a relay PortLike
// (guest↔host) and a plain MessageChannel (the local iframe proxy), since either
// delivers the packed Uint8Array intact.

import * as pack from 'packcat';
import type { Filesystem, FilesystemSnapshot, FsChange, FsStat } from './interface';

/** a MessagePort-shaped duplex: `onmessage`, `postMessage`, and `close`. A relay
 *  port and a real MessagePort are interchangeable through this (matches the relay
 *  transport's PortLike structurally). */
