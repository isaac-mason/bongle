...
import { Channel, type ClientUser, type ResolvedAvatar } from 'bongle/interface';
...
// recipe; the pieces that vary by host (fs, the game transport, how to reach the
// embedding platform) are the named functions below, with caps from `env`.
//
// Engine RUNTIME is reached only via the runner (engine.ts); statics are leaf
// utilities bundled at engine build.

type EngineClientApi = typeof import('bongle/engine-client')['EngineClient'];
type EngineClientEditorApi = typeof import('bongle/engine-client-editor');
type ClientState = ReturnType<EngineClientApi['init']>;
...
const client: App<AppInit> = async (env) => {
    ...
    try {
        ...
        await injectEngineStylesheet(fs);

        progress('loading engine');
        await importEngine(runner, 'client', env.init.entry ?? 'src/index.ts');
        const { EngineClient } = (await runner.import('bongle/engine-client')) as { EngineClient: EngineClientApi };
        const EngineClientEditor = (await runner.import('bongle/engine-client-editor')) as EngineClientEditorApi;
        ...
        progress('booting');
        EngineClientEditor.setup(state, { sceneSource: fsSceneSource(fs) });
        mark('editor ui mounted');
        // Everything from here reads bake outputs: the generated barrels (baked bin
        // paths on the model handles, scene payloads) and the atlases `load` fetches.
        // The client is spawned while the first bake runs, so this is where it waits
        // for it; the engine import, init and the UI mount have already overlapped it.
        ...
        // the debug pane's latency sim sits between the game transport and the engine.
        let game: { send: (bytes: Uint8Array) => void } = { send: () => {} };
        const netSim = editorNetSim<Uint8Array>(EngineClientEditor.useEditor, {
            deliverInbound: (bytes) => EngineClient.receive(state, Channel.RELIABLE, bytes),
            deliverOutbound: (bytes) => game.send(bytes),
        });

        // wire receive first, then take `send`, so a snapshot sent at join time isn't
        // dropped before the handler exists. Parks until 'game' is served: the client
        // is spawned while the stack is still coming up, and a server that arrives
        // later (a fixed src/ + restart) is joined then. A crashed server is reported
        // by the boot supervisor, not by a timeout.
        ...
        // release due inbound, advance (which sends), release due outbound.
        frameLoop(
            (dt, now) => {
                netSim.pump(now);
            ...
            { onError: (message) => err(message), onFirstFrame: () => mark('first frame rendered') },
        );
        progress('live');
...
async function injectEngineStylesheet(fs: Filesystem): Promise<void> {
    try {
