...
describe('diffSync — threshold', () => {
    ...
    it('emits only on significant change; sub-threshold accumulates against the last emit', () => {
        ...
        expect(diffSync(posDef(), codec, inst as never, node, 0, s, false)).toBe(false);
        ...
        inst.pos = [1.0, 0, 0];
        expect(diffSync(posDef(), codec, inst as never, node, 0, s, false)).toBe(true);
    ...
    it('client emits the first-seen threshold value', () => {
        ...
        const s = syncState();
        expect(diffSync(posDef(), codec, inst as never, node, 0, s, true)).toBe(true);
    });
});
