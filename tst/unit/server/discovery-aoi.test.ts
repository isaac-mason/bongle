...
describe('discovery — region-tied node AOI', () => {
    ...
    it('completeness: an ancestor gaining a transform re-files the index (descendant root-status flips)', () => {
        ...
        expect(st.regions.rootToRegion.has(child)).toBe(true);
        expect(st.regions.rootToRegion.has(parent)).toBe(false);
        ...
        flush(discovery, server.rooms, resources);
        expect(st.regions.rootToRegion.has(parent)).toBe(true);
        expect(st.regions.rootToRegion.has(child)).toBe(false);
