...
describe('discovery — region-tied node AOI', () => {
    ...
    it('completeness: an ancestor gaining a transform re-files the index (descendant root-status flips)', () => {
        ...
        expect(st.regions.ofRoot.has(child)).toBe(true);
        expect(st.regions.ofRoot.has(parent)).toBe(false);
        ...
        flush(discovery, server.rooms, resources);
        expect(st.regions.ofRoot.has(parent)).toBe(true);
        expect(st.regions.ofRoot.has(child)).toBe(false);
