...
//
// editor/build/env-plugin.ts — compile-time env.<key> replacement for the
// engine bundle. Copied from kit/env-plugin.ts (kit is being dismantled; the
// editor owns its build). Each realm build applies its own env values:
// client iframe = {client:true}, server worker = {server:true}, pipeline =
// {server:true, editor:true}.
