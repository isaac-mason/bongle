...
function resolvePaths(projectDir: string): Paths {
    return {
        clientBinDir: path.join(projectDir, 'resources', 'client', 'models'),
        serverBinDir: path.join(projectDir, 'resources', 'server', 'models'),
        barrelPath: path.join(projectDir, 'src', 'generated', 'models.ts'),
    };
}
...
export async function buildModels(module: ModuleVersion, opts: BuildModelsOptions): Promise<boolean> {
    ...
    for (const [id, def] of models) {
        const e = await processModel(id, def.src, cache, paths, opts.projectDir);
        if (!e) continue;
        entries.push(e);
    ...
    for (const e of entries) {
        liveBins.add(path.basename(e.clientBinPath));
        liveBins.add(path.basename(e.serverBinPath));
    }
    gcOrphanBins(paths, liveBins);
 ...
 */
async function loadAndOptimize(absPath: string): Promise<Document> {
...
async function processModel(
    ...
    const absPath = resolveSrcToAbsPath(srcRel, projectDir);
    ...
    if (cached && cached.srcHash === srcHash) {
        const clientBinPath = path.join(paths.clientBinDir, path.basename(cached.clientBin));
        const serverBinPath = path.join(paths.serverBinDir, path.basename(cached.serverBin));
        if (fs.existsSync(clientBinPath) && fs.existsSync(serverBinPath)) {
            const doc = await loadAndOptimize(absPath);
            const { payload } = projectDocument(doc);
            return {
                id,
                srcRel,
                srcHash,
    ...
    const hash8 = sha256Hex(clientBytes).slice(0, 8);
    const clientFilename = `${id}.${hash8}.client.bin`;
    const serverFilename = `${id}.${hash8}.server.bin`;
    const clientBinPath = path.join(paths.clientBinDir, clientFilename);
    const serverBinPath = path.join(paths.serverBinDir, serverFilename);

    ensureDir(paths.clientBinDir);
    ensureDir(paths.serverBinDir);
    fs.writeFileSync(clientBinPath, clientBytes);
    fs.writeFileSync(serverBinPath, serverBytes);
...
function projectDocument(doc: Document): ProjectedDocument {
...
function emitBarrel(paths: Paths, entries: BuildEntry[]): void {
    ensureDir(path.dirname(paths.barrelPath));
...
function gcOrphanBins(paths: Paths, live: Set<string>): void {
    for (const dir of [paths.clientBinDir, paths.serverBinDir]) {
