...
console.log(`${'identity%'.padStart(9)} ${'always ns'.padStart(10)} ${'skip ns'.padStart(9)} ${'saving'.padStart(8)}`);

for (const frac of [0, 0.125, 0.25, 0.5, 1]) {
    ...
    console.log(
        `${(frac * 100).toFixed(1).padStart(9)} ${(always / N).toFixed(2).padStart(10)} ${(skip / N).toFixed(2).padStart(9)} ${(((always - skip) / always) * 100).toFixed(1).padStart(7)}%`,
    );
}
console.log();
