...
export async function newProject(targetDir: string, opts: NewOptions = {}) {
    ...
    const bongleVersion = readEngineVersion(kitDir);
    rewritePackageJson(path.join(target, 'package.json'), projectName, bongleVersion);
...
function rewritePackageJson(file: string, projectName: string, bongleVersion: string) {
    const raw = fs.readFileSync(file, 'utf8');
    const replaced = raw
        .replace('__PROJECT_NAME__', projectName)
        .replace('__BONGLE_VERSION__', bongleVersion);
    fs.writeFileSync(file, replaced);
}

function readEngineVersion(kitDir: string): string {
    const pkgFile = path.join(kitDir, '..', 'package.json');
