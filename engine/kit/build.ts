 ...
 *  consuming project invokes us. `pkgPath` may be `'../package.json'`
 *  (kit itself) or a workspace dep like `'@bongle/interface/package.json'`. */
...
export async function build(projectDir: string) {
    ...
    // runtime in lib/engine/src/* assumes content is at latest.
    ...
    const manifest = buildManifest({
        ...
        kitVersion: readPackageVersion('../package.json'),
        bongleVersion: readPackageVersion('bongle/package.json'),
        interfaceVersion: readPackageVersion('@bongle/interface/package.json'),
        matchmaking,
    });
